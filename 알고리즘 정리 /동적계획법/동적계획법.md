...
## 예시: 0-1 배낭문제 (Knapsack Problem)

### 문제 요약
- 물건 N개, 각각 무게 W[i], 가치 V[i]
- 최대 무게 K 이하로 담을 때 가치의 합이 최대가 되도록 하라.

### 상태 정의
| 이름      | 의미                              |
| ------- | ------------------------------- |
| `dp[w]` | 현재까지 고려한 물건으로 무게가 `w`일 때의 최대 가치 |
...
- 단, w - W[i] >= 0 일 때만 적용
- 물건은 “담는다 / 안 담는다” 두 가지 선택이므로 “0-1 배낭”
...
### 이 문제도 구조는 같음
- “현재 dp 상태” → “새로운 물건 포함 시 next 상태”로 갱신
- 차이점은
   - 나머지 대신 무게가 상태
   - 덧셈 대신 최댓값 비교로 전이
...
| 구분          | 3의 배수 문제               | 5의 배수 문제               | 배낭 문제                               |
| ----------- | ---------------------- | ---------------------- | ----------------------------------- |
| **dp 의미**   | 합 % 3 == r 인 개수        | 합 % 5 == r 인 개수        | 무게 w 일 때 최대 가치                      |
| **상태 수**    | 3                      | 5                      | 최대 무게 K+1                           |
| **전이식**     | `next[(r+m)%3]+=dp[r]` | `next[(r+m)%5]+=dp[r]` | `dp[w]=max(dp[w], dp[w-W[i]]+V[i])` |
| **연산 방식**   | 개수 합산(+)               | 개수 합산(+)               | 최댓값 비교(max)                         |
| **공집합 초기화** | dp[0]=1                | dp[0]=1                | dp[0]=0                             |
| **결과**      | dp[0]                  | dp[0]                  | dp[K]                               |

---

## 이렇게 두 문제를 비교해 보면 
 **구조(틀)** — `현재 dp → next 계산 → dp = next` 동일  
 **차이점** — “dp의 의미”와 “전이식 계산 로직”만 다름  

즉, **DP는 문제마다 점화식이 다르지만, 구조적 흐름은 같다**는 걸 직접 체감할 수 있다.
