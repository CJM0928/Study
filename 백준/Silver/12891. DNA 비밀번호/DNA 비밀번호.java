...
public class Main {
    ...
    private void PasswordPrint(){
        kb = new Scanner(System.in);

        int S = kb.nextInt();   // password 전체 길이
        int P = kb.nextInt();   // 조건 password 길이

        A = new char[S];

        Password = kb.next();    // 임의 패스워드

        for(int i=0; i<4; i++){
            ACount[i] = kb.nextInt();   // {A C G T} 최소개수
            if(ACount[i] == 0){
                Check++;
            }
        }
